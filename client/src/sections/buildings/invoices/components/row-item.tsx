...
import type { IInvoice } from '../api/get';

export const InvoiceRowItem = ({ row, remove, edit }: RowItemProps<IInvoice>) => {
  const { t } = useTranslate();

  return (
    <TableRow hover>
      <TableCell>{row.document_number || '-'}</TableCell>
      <TableCell>{row.document_date ? dayjs(row.document_date).format('DD.MM.YYYY') : '-'}</TableCell>
