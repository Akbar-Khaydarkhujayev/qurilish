...
export const getAll = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const searchClause = buildSearchClause(['object_card.object_name', 'object_card.card_number', 'object_card.address'], search as string);
    ...
    responseFormatter.success(res, result.rows, 'Object cards retrieved successfully', 200, calculateMeta(page, limit, total));
  } catch (error) {
...
export const getById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    responseFormatter.success(res, result.rows[0], 'Object card retrieved successfully');
  } catch (error) {
...
export const getSummary = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    responseFormatter.success(res, summary, 'Object card summary retrieved successfully');
  } catch (error) {
...
export const create = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!object_name || !region_id || !district_id || !project_organization_id || !contractor_id || !construction_status_id || !organization_id) {
      responseFormatter.badRequest(res, 'Object name, region, district, project organization, contractor, organization, and status are required');
      return;
    }

    // Validate region exists
    const regionCheck = await pool.query('SELECT id FROM regions WHERE id = $1 AND is_deleted = FALSE', [region_id]);
    ...
    const districtCheck = await pool.query('SELECT id FROM districts WHERE id = $1 AND is_deleted = FALSE', [district_id]);
    if (districtCheck.rows.length === 0) {
      responseFormatter.badRequest(res, 'Invalid district ID');
      return;
    }

    // Validate project organization exists
    const projectOrgCheck = await pool.query('SELECT id FROM project_organization WHERE id = $1 AND is_deleted = FALSE', [project_organization_id]);
    ...
    const contractorCheck = await pool.query('SELECT id FROM contractor WHERE id = $1 AND is_deleted = FALSE', [contractor_id]);
    ...
    const statusCheck = await pool.query('SELECT id FROM construction_status WHERE id = $1 AND is_deleted = FALSE', [construction_status_id]);
    ...
    if (technical_supervisor_id) {
      const supervisorCheck = await pool.query('SELECT id FROM users WHERE id = $1 AND is_deleted = FALSE', [technical_supervisor_id]);
    ...
    const organizationCheck = await pool.query('SELECT id FROM organizations WHERE id = $1 AND is_deleted = FALSE', [organization_id]);
    ...
    if (card_number) {
      ...
      if (cardNumberCheck.rows.length > 0) {
        responseFormatter.conflict(res, 'Object card with this card number already exists');
        return;
      }
    }
    ...
    responseFormatter.created(res, result.rows[0], 'Object card created successfully');
  } catch (error) {
...
export const update = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    responseFormatter.success(res, result.rows[0], 'Object card updated successfully');
  } catch (error) {
