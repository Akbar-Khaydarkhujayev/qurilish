import { Response } from 'express';
import pool from '../config/database';
import { AuthRequest } from '../types';
import * as responseFormatter from '../utils/responseFormatter';
import { parsePagination, buildOrderClause, calculateMeta, buildSearchClause } from '../utils/queryBuilder';

export const getAll = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { page, limit, offset } = parsePagination(req.query);
    const { sort_by, sort_order, search, start_date, end_date } = req.query;

    const searchClause = buildSearchClause(['document_number', 'description'], search as string);
    const orderClause = buildOrderClause(sort_by as string, sort_order as string);

    let whereClause = 'WHERE invoices.is_deleted = FALSE';
    const params: any[] = [];

    // Filter by date range
    if (start_date) {
      whereClause += ` AND invoices.document_date >= $${params.length + 1}`;
      params.push(start_date);
    }

    if (end_date) {
      whereClause += ` AND invoices.document_date <= $${params.length + 1}`;
      params.push(end_date);
    }

    if (searchClause.clause) {
      whereClause += ` AND ${searchClause.clause}`;
      params.push(...searchClause.params);
    }

    const countResult = await pool.query(
      `SELECT COUNT(*) FROM invoices ${whereClause}`,
      params
    );
    const total = parseInt(countResult.rows[0].count);

    const result = await pool.query(
      `SELECT
        invoices.*,
        object_card.object_name,
        object_card.card_number,
        object_contract.contract_number,
        object_contract.contract_date
       FROM invoices
       LEFT JOIN object_card ON invoices.object_card_id = object_card.id
       LEFT JOIN object_contract ON invoices.object_contract_id = object_contract.id
       ${whereClause}
       ${orderClause}
       LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, limit, offset]
    );

    // Calculate total amount
    const totalAmountResult = await pool.query(
      `SELECT SUM(amount) as total_amount FROM invoices ${whereClause}`,
      params
    );

    const totalAmount = parseFloat(totalAmountResult.rows[0].total_amount) || 0;

    responseFormatter.success(
      res,
      {
        invoices: result.rows,
        totalAmount,
      },
      'Invoices retrieved successfully',
      200,
      calculateMeta(page, limit, total)
    );
  } catch (error) {
    console.error('Get invoices error:', error);
    responseFormatter.error(res, 'Error retrieving invoices');
  }
};

export const getById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      `SELECT
        invoices.*,
        object_card.object_name,
        object_card.card_number,
        object_card.address,
        object_card.construction_cost,
        object_contract.contract_number,
        object_contract.contract_date,
        object_contract.contract_amount
       FROM invoices
       LEFT JOIN object_card ON invoices.object_card_id = object_card.id
       LEFT JOIN object_contract ON invoices.object_contract_id = object_contract.id
       WHERE invoices.id = $1 AND invoices.is_deleted = FALSE`,
      [id]
    );

    if (result.rows.length === 0) {
      responseFormatter.notFound(res, 'Invoice not found');
      return;
    }

    responseFormatter.success(res, result.rows[0], 'Invoice retrieved successfully');
  } catch (error) {
    console.error('Get invoice error:', error);
    responseFormatter.error(res, 'Error retrieving invoice');
  }
};

export const getByObjectCardId = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { objectCardId } = req.params;

    // Verify object card exists
    const objectCardCheck = await pool.query(
      'SELECT id FROM object_card WHERE id = $1 AND is_deleted = FALSE',
      [objectCardId]
    );

    if (objectCardCheck.rows.length === 0) {
      responseFormatter.notFound(res, 'Object card not found');
      return;
    }

    const result = await pool.query(
      `SELECT
        invoices.*,
        object_contract.contract_number,
        object_contract.contract_date
       FROM invoices
       LEFT JOIN object_contract ON invoices.object_contract_id = object_contract.id
       WHERE invoices.object_card_id = $1 AND invoices.is_deleted = FALSE
       ORDER BY invoices.document_date DESC NULLS LAST`,
      [objectCardId]
    );

    // Calculate total amount for this object card
    const totalAmountResult = await pool.query(
      `SELECT SUM(amount) as total_amount
       FROM invoices
       WHERE object_card_id = $1 AND is_deleted = FALSE`,
      [objectCardId]
    );

    const totalAmount = parseFloat(totalAmountResult.rows[0].total_amount) || 0;

    responseFormatter.success(
      res,
      {
        invoices: result.rows,
        totalAmount,
        count: result.rows.length,
      },
      'Invoices retrieved successfully'
    );
  } catch (error) {
    console.error('Get invoices by object card error:', error);
    responseFormatter.error(res, 'Error retrieving invoices');
  }
};

export const create = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { object_card_id, object_contract_id, document_number, document_date, amount, description } = req.body;

    if (!object_card_id || !object_contract_id) {
      responseFormatter.badRequest(res, 'Object card ID and object contract ID are required');
      return;
    }

    // Verify object card exists
    const objectCardCheck = await pool.query(
      'SELECT id FROM object_card WHERE id = $1 AND is_deleted = FALSE',
      [object_card_id]
    );

    if (objectCardCheck.rows.length === 0) {
      responseFormatter.badRequest(res, 'Invalid object card ID');
      return;
    }

    // Verify object contract exists and belongs to the object card
    const contractCheck = await pool.query(
      'SELECT id FROM object_contract WHERE id = $1 AND object_card_id = $2 AND is_deleted = FALSE',
      [object_contract_id, object_card_id]
    );

    if (contractCheck.rows.length === 0) {
      responseFormatter.badRequest(res, 'Invalid object contract ID or contract does not belong to this object card');
      return;
    }

    // Validate amount is positive if provided
    if (amount !== undefined && amount !== null && parseFloat(amount) < 0) {
      responseFormatter.badRequest(res, 'Amount must be a positive number');
      return;
    }

    // Check for duplicate document_number if provided
    if (document_number) {
      const duplicateCheck = await pool.query(
        'SELECT id FROM invoices WHERE document_number = $1 AND object_card_id = $2 AND is_deleted = FALSE',
        [document_number, object_card_id]
      );

      if (duplicateCheck.rows.length > 0) {
        responseFormatter.conflict(res, 'Invoice with this document number already exists for this object card');
        return;
      }
    }

    const result = await pool.query(
      `INSERT INTO invoices (object_card_id, object_contract_id, document_number, document_date, amount, description)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [
        object_card_id,
        object_contract_id,
        document_number || null,
        document_date || null,
        amount || null,
        description || null,
      ]
    );

    responseFormatter.created(res, result.rows[0], 'Invoice created successfully');
  } catch (error) {
    console.error('Create invoice error:', error);
    responseFormatter.error(res, 'Error creating invoice');
  }
};

export const update = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { object_card_id, object_contract_id, document_number, document_date, amount, description } = req.body;

    if (!object_card_id || !object_contract_id) {
      responseFormatter.badRequest(res, 'Object card ID and object contract ID are required');
      return;
    }

    // Verify object card exists
    const objectCardCheck = await pool.query(
      'SELECT id FROM object_card WHERE id = $1 AND is_deleted = FALSE',
      [object_card_id]
    );

    if (objectCardCheck.rows.length === 0) {
      responseFormatter.badRequest(res, 'Invalid object card ID');
      return;
    }

    // Verify object contract exists and belongs to the object card
    const contractCheck = await pool.query(
      'SELECT id FROM object_contract WHERE id = $1 AND object_card_id = $2 AND is_deleted = FALSE',
      [object_contract_id, object_card_id]
    );

    if (contractCheck.rows.length === 0) {
      responseFormatter.badRequest(res, 'Invalid object contract ID or contract does not belong to this object card');
      return;
    }

    // Validate amount is positive if provided
    if (amount !== undefined && amount !== null && parseFloat(amount) < 0) {
      responseFormatter.badRequest(res, 'Amount must be a positive number');
      return;
    }

    // Check for duplicate document_number if provided (excluding current invoice)
    if (document_number) {
      const duplicateCheck = await pool.query(
        'SELECT id FROM invoices WHERE document_number = $1 AND object_card_id = $2 AND id != $3 AND is_deleted = FALSE',
        [document_number, object_card_id, id]
      );

      if (duplicateCheck.rows.length > 0) {
        responseFormatter.conflict(res, 'Invoice with this document number already exists for this object card');
        return;
      }
    }

    const result = await pool.query(
      `UPDATE invoices
       SET object_card_id = $1, object_contract_id = $2, document_number = $3, document_date = $4, amount = $5, description = $6
       WHERE id = $7 AND is_deleted = FALSE
       RETURNING *`,
      [
        object_card_id,
        object_contract_id,
        document_number || null,
        document_date || null,
        amount || null,
        description || null,
        id,
      ]
    );

    if (result.rows.length === 0) {
      responseFormatter.notFound(res, 'Invoice not found');
      return;
    }

    responseFormatter.success(res, result.rows[0], 'Invoice updated successfully');
  } catch (error) {
    console.error('Update invoice error:', error);
    responseFormatter.error(res, 'Error updating invoice');
  }
};

export const remove = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      'UPDATE invoices SET is_deleted = TRUE WHERE id = $1 AND is_deleted = FALSE RETURNING id',
      [id]
    );

    if (result.rows.length === 0) {
      responseFormatter.notFound(res, 'Invoice not found');
      return;
    }

    responseFormatter.success(res, null, 'Invoice deleted successfully');
  } catch (error) {
    console.error('Delete invoice error:', error);
    responseFormatter.error(res, 'Error deleting invoice');
  }
};
