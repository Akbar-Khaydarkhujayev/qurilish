  ...
  try {
    ...
    if (!username || !password || !name || !organizationId) {
      responseFormatter.badRequest(
        res,
        "Username, password, name, and organization ID are required"
      );
      return;
    }
...
export const createUser = async (
  ...
  try {
    ...
    if (!username || !password || !name || !organizationId || !role) {
      responseFormatter.badRequest(
        res,
        "Username, password, name, organization ID, and role are required"
      );
      return;
    }
    ...
    if (!validRoles.includes(role as UserRole)) {
      responseFormatter.badRequest(
        res,
        "Invalid role. Must be one of: super_admin, region_admin, user"
      );
      return;
    }
    ...
    if (!canCreateUser(creator.role as UserRole, role as UserRole)) {
      responseFormatter.forbidden(
        res,
        `${creator.role} can only create ${
          creator.role === "super_admin" ? "region_admin" : "user"
        } users`
      );
      return;
    }
    ...
    if (role === "region_admin" && !regionId) {
      responseFormatter.badRequest(
        res,
        "Region ID is required for region_admin role"
      );
      return;
    }
    ...
    if (role === "user" && creator.role === "region_admin") {
      if (!regionId || regionId !== creator.organization_id) {
        responseFormatter.badRequest(
          res,
          "User must be assigned to your region"
        );
        return;
      }
    }
    ...
    const result = await pool.query(
      ...
      [
        ...
        regionId || null,
        role,
        role === "user" ? "Technical Supervisor" : role,
      ]
    );
...
export const signIn = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (result.rows.length === 0) {
      responseFormatter.unauthorized(res, "Invalid username or password");
      return;
    }
